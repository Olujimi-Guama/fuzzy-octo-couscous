...
object chiefroom = load_object(ROOM+ "chieftain");
...
int openDoor(object who) {
    if(GetSprung()){
        ...
        say(this_player()->GetName()+
          " pulls the lever, and the floor drops out!");
        tell_room(ROOM+ "cavetroll","The ceiling opens up.");
        chiefroom->SetMedium(MEDIUM_AIR);
        chiefroom->AddExit("down", ROOM+ "cavetroll");
...
int closeDoor(object who){
    ...
    if(!GetSprung()){
        ...
        say(this_player()->GetName()+
          " pushes the lever, and the floor closes.");
        tell_room(ROOM+ "cavetroll", "The ceiling closes.");
