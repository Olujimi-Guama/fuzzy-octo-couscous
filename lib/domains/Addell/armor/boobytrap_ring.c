...
void boobytrap_me(){
    ...
    /* Avoids boobytrapping the blueprint ob */
...
static void create(){
    ...
    SetShort("beautiful silver ring");
    SetLong(
      "This is a simple but beautiful ring made of "
      "silver, suitable for wearing on one's finger. "
      "The ring bears an inscription.");
    SetMass(1);
    SetBaseCost("silver", 10);
    SetArmorType(A_RING);
    SetItems(([
        ({"script","words","inscription","writing"}) : "Words "
        "engraved on the ring in a highly stylized script."
      ]) );
    SetRead(({"script","words","inscription",
        "writing"}), "\"bad wolf\""  );
